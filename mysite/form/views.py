...
from mysite.decorators import user_is_QC_Spain

def check_for_done(p):
...
def add_to_histo_background(file):
    message = 'error'

    # file = r"C:\Users\cjulemont\PycharmProjects\Django_apache_channel\mysite\media\Hoja1_validate_2_vsfUFlO.xlsx"
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        # to demonstrate it will take a batch of 4 jobs at the same time
        cmds = [[r"C:\Users\cjulemont\PycharmProjects\Channel_dynamic\venv\Scripts\python.exe",
                 r"C:\Users\cjulemont\PycharmProjects\Channel_dynamic\Main_add_to_history.py", file]]
        start = time.time()
        conn = sqlite3.connect(r'/home/cwayacita/Documents/Pycharmprojects/website_company_python39/mysite/db.sqlite3')
        file = 'test'
        sql_query = """INSERT INTO tasks (id, name, status_id, begin_date, end_date )
...
def simple_upload(request):
    ...
    if request.method == 'GET' and request.path == '/form/media/':
        conn = sqlite3.connect(r'/home/cwayacita/Documents/Pycharmprojects/website_company_python39/mysite/db.sqlite3')
        df = pd.read_sql_query("SELECT * from tasks;", conn)
        print(df)
        if df.empty:
            # messages.warning(request, 'No file are actually in treatment')
            return render(request, 'form/simple_upload2.html', {'status_url': status_url})
        else:
        ...
        if uploaded_file_url:
            input_folder = r'C:\Users\cjulemont\PycharmProjects\Django_apache_channel\mysite\media'
            ...
            return render(request, 'form/simple_upload2.html', )



@login_required
@user_is_QC_Spain
def simple_upload_validated(request):
    status_url = ''

    if request.method == 'GET' and request.path == '/form/media/validated/':
        conn = sqlite3.connect(r'/home/cwayacita/Documents/Pycharmprojects/website_company_python39/mysite/db.sqlite3')
        df = pd.read_sql_query("SELECT * from tasks;", conn)
        print(df)
        ...
        if df.empty:
            # messages.warning(request, 'No file are actually in treatment')
            return render(request, 'form/simple_upload5.html', {'status_url': status_url})
        else:
            messages.warning(request, 'The file is being treated, please wait')
            return render(request, 'form/simple_upload3.html')
        ...
        if uploaded_file_url:
            ...
            try:
                ...
                message = 'error'
            try:
                df = df[['fecha', 'cl', 'local_brand', 'mensaje_literal_del_panelista',
                   'match_type', 'retrieve_from_histo', 'keep_predicted', 'final_status',
                   'predicted_A_final', 'predicted_B_final', 'predicted_C_final']]
            except:
                messages.warning(request,  'Please modify the columns headers :' "'fecha', 'cl', 'local_brand', 'mensaje_literal_del_panelista',       'match_type', 'retrieve_from_histo', 'keep_predicted', 'final_status',       'predicted_A_final', 'predicted_B_final', 'predicted_C_final'" )
                message = 'error'

            if message in ('error', 'warning'):
                messages.warning(request,  'The format of your file is wrong, please retry')
            else :
                message = add_to_histo_background(newest)


            return render(request, 'form/simple_upload5.html', )

# df = pd.read_excel(r"C:\Users\cjulemont\PycharmProjects\Django_apache_channel\mysite\media\Hoja1_validate_2_vsfUFlO.xlsx", sheet_name='Hoja1')
# df_new = df.dropna(subset=['predicted_A_final'])
#
